...
test("后台对账在供应商成功后结算积分并登记视频资产", async () => {
  ...
  const ledger = {
    ...
    settleInTransaction: async (
      _transaction: unknown,
      input: { reservedMilli: bigint; chargedMilli: bigint },
    ) => {
      settled += 1;
  ...
  const upstream = {
    pollVideo: async () => ({
      ...
      data: [
        {
          url: "https://metajing.cn/video-media/result.mp4",
          mp4_url: "https://metajing.cn/video-media/result.mp4",
        },
      ],
    }),
    videoMedia: async () => ({
  ...
  const canvasMedia = {
    saveBufferForWorkspace: async () => ({ mimeType: "video/mp4", bytes: 5 }),
    saveVideoBufferForWorkspace: async () => ({
      mimeType: "video/mp4",
      bytes: 5,
      checksum: "abcdef0123456789fedcba98765432100",
    }),
  };
  ...
  assert.equal(
    (updates[0] as { data: { status: GenerationStatus } }).data.status,
    GenerationStatus.SUCCEEDED,
  );
  assert.equal(assets.length, 1);
  assert.equal(
    (assets[0] as { create: { data: { url: string } } }).create.data.url,
    "/platform-api/canvas-media/video%3AGEN-VIDEO-1?v=abcdef0123456789",
  );
});
...
test("后台查询暂时失败时保留预授权，不结算也不退款", async () => {
  ...
  const prisma = {
    ...
    asset: {
      findFirst: async () => null,
      upsert: async () => assert.fail("查询失败时不应登记资产"),
    },
  } as unknown as PrismaService;
  const ledger = {
    runSerializable: async () => {
...
test("视频资产暂时无法登记时保留任务待重试，不提前扣费", async () => {
  ...
  const upstream = {
    pollVideo: async () => ({
      state: "success",
      result_url: "https://metajing.cn/video-media/result.mp4",
    }),
    videoMedia: async () => ({
      body: Buffer.from("video"),
      contentType: "video/mp4",
...
test("后台视频对账按配置限制并发查询", async (context) => {
  ...
  context.after(() => {
    if (previousConcurrency === undefined)
      delete process.env.VIDEO_RECONCILE_CONCURRENCY;
  ...
  let peak = 0;
  const prisma = {
    generationJob: { findMany: async () => jobs },
  } as unknown as PrismaService;
  const upstream = {
    pollVideo: async () => {
      active += 1;
