...
import {
  canvasMediaUrl,
  CanvasMediaService,
} from "../canvas-media/canvas-media.service";
import { PrismaService } from "../prisma/prisma.service";
import { LedgerService } from "../wallet/ledger.service";
import {
  findProviderModel,
  isExclusiveVideoModelId,
  ...
  implements OnApplicationBootstrap, OnModuleDestroy
{
  ...
  onApplicationBootstrap() {
    const intervalMs = Math.max(
      5_000,
      Number(process.env.VIDEO_RECONCILE_INTERVAL_MS) || 10_000,
    );
    this.reconcileTimer = setInterval(
      () => void this.reconcileSubmittedVideos(),
      intervalMs,
    );
  ...
  parseImage(body: unknown) {
    return parseSchema(imageRequestSchema, body);
  }
  parseVideo(body: unknown) {
    return parseSchema(videoRequestSchema, body);
  }
  parseSpeech(body: unknown) {
    return parseSchema(minimaxSpeechSchema, body);
  }
  ...
  async image(
    ...
    try {
      ...
      const assets = await this.registerImageAssets(
        reservation.job,
        result,
        input.output_format,
      );
      setTimeout(() => {
        void this.persistGeneratedImages(reservation.job, result, assets).catch(
          (error) => {
            this.logger.warn(
              `图片任务 ${reservation.job.publicId} 后台持久化失败：${error instanceof Error ? error.message : "未知错误"}`,
            );
          },
        );
      }, 5_000).unref();
      return {
        result: rewriteGeneratedImageUrls(result, assets),
        jobId: reservation.job.publicId,
        chargedCredits: quote.retailCredits,
      };
    } catch (error) {
  ...
  async createVideo(
    ...
    if (
      ...
      !isExclusiveVideoModelId(input.model)
    )
      throw new BadRequestException("该视频模型已停用，未调用上游且不会扣费");
    const resolution = resolveProviderVideoResolution(model, input.resolution);
    if (!resolution)
      throw new BadRequestException(
        `${model.label} 不支持 ${input.resolution || "当前"} 分辨率`,
      );
    ...
    const quote = this.pricing.quote(
      ...
      resolution,
      { hasVideoReferences: normalizedInput.videos.length > 0 },
    );
    const reservation = await this.reserve(
      userId,
      workspace.id,
      ...
      quote,
      summarizeVideo(normalizedInput),
    );
    if (!reservation.created) {
      if (
        reservation.job.status === GenerationStatus.SUBMITTED ||
        reservation.job.status === GenerationStatus.SUCCEEDED
      )
        return {
          id: reservation.job.publicId,
          state: reservation.job.providerState || "submitted",
        };
    ...
    try {
      ...
      if (!providerTaskId)
        throw new ConflictException("Seedance 没有返回任务号");
      const job = await this.prisma.generationJob.update({
        where: { id: reservation.job.id },
        data: {
          providerTaskId,
          status: GenerationStatus.SUBMITTED,
          providerState: String(
            result.state || result.status || "submitted",
          ).slice(0, 100),
        },
      });
  ...
  async pollVideo(userId: string, publicId: string) {
    const job = await this.prisma.generationJob.findFirst({
      where: { publicId, userId, capability: "video" },
    });
    if (!job) throw new NotFoundException("视频任务不存在");
    if (
      job.status === GenerationStatus.FAILED ||
      job.status === GenerationStatus.RELEASED
    )
      return {
        id: job.publicId,
        state: "failed",
    ...
    if (job.status === GenerationStatus.SUCCEEDED)
      return this.completedVideoResult(job);
    if (!job.providerTaskId)
      throw new ConflictException("视频任务尚未提交到供应商");
  ...
  async reconcileSubmittedVideos() {
    ...
    try {
      ...
      const concurrency = Math.max(
        1,
        Math.min(8, Number(process.env.VIDEO_RECONCILE_CONCURRENCY) || 4),
      );
        ...
        await Promise.all(
          batch.map(async (job) => {
            try {
              ...
              this.logger.warn(
                `视频任务 ${job.publicId} 后台查询失败：${error instanceof Error ? error.message : "未知错误"}`,
              );
            }
          }),
        );
  ...
  private async refreshSubmittedVideo(job: GenerationJob) {
    ...
    const state = String(
      result.state || result.status || "processing",
    ).toLowerCase();
    ...
    if (videoFailed(result, state)) {
      await this.release(
        job,
        new Error(readResultError(result) || "Seedance 视频生成失败"),
      );
      return { ...withPublicId, state: state || "failed", is_final: true };
    }
    await this.prisma.generationJob.updateMany({
      where: { id: job.id, status: GenerationStatus.SUBMITTED },
      data: { providerState: state.slice(0, 100) },
    });
    return withPublicId;
  }
  ...
  private async completedVideoResult(job: GenerationJob) {
    const asset = await this.prisma.asset.findFirst({
      where: { generationJobId: job.id, kind: AssetKind.VIDEO },
      orderBy: { ordinal: "asc" },
      select: { data: true },
    });
    const data = asRecord(asset?.data);
    let resultUrl = String(data.url || "").trim();
    if (/^https?:\/\//i.test(resultUrl) && !String(data.storageKey || ""))
      resultUrl = await this.registerVideoAsset(job, resultUrl);
    return {
      id: job.publicId,
      state: "success",
      status: "已完成",
  ...
  async importVideoMedia(
    ...
    if (!/^https?:\/\//i.test(sourceUrl))
      throw new BadRequestException("只能修复 MetaJing 返回的视频地址");
    const media = await this.upstream.videoMedia(sourceUrl);
    const storageKey = `video:repair_${randomBytes(12).toString("hex")}`;
    const stored = await this.canvasMedia.saveVideoBufferForWorkspace(
      userId,
      workspace.id,
      storageKey,
      media.contentType,
  ...
  async speech(
    ...
    try {
      ...
      return {
        ...output,
        jobId: reservation.job.publicId,
        chargedCredits: actualQuote.retailCredits,
      };
    } catch (error) {
      await this.release(reservation.job, error);
      throw error;
  ...
  async imageAssetStream(
    ...
    if (storageKey) {
      try {
        const local = await this.canvasMedia.open(
          userId,
          workspacePublicId,
          storageKey,
        );
        return {
          ...
          contentType: local.media.mimeType,
          contentLength: Number(local.media.bytes),
        };
      } catch (error) {
  ...
  private assertFreshSyncReservation(reservation: {
    job: GenerationJob;
    created: boolean;
  }) {
    if (!reservation.created)
      throw new ConflictException(
        `请求 ${reservation.job.publicId} 已处理；同步生成结果不会被重复返回`,
      );
  }
  ...
  private async settle(job: GenerationJob, chargedMilli: bigint) {
    await this.ledger.runSerializable(async (tx) => {
      ...
      if (
        ...
        current.status === GenerationStatus.RELEASED
      )
        throw new ConflictException("任务已退款，不能再次结算");
      ...
      await tx.generationJob.update({
        ...
        data: {
          ...
          chargedMilliCredits: chargedMilli,
  ...
  private async release(job: GenerationJob, error: unknown) {
    const message =
      error instanceof Error ? error.message.slice(0, 1_000) : "供应商生成失败";
    await this.ledger.runSerializable(async (tx) => {
      const current = await tx.generationJob.findUniqueOrThrow({
        where: { id: job.id },
      });
      if (
        current.status === GenerationStatus.FAILED ||
        current.status === GenerationStatus.RELEASED ||
        current.status === GenerationStatus.SUCCEEDED
      )
        return;
  ...
  private async registerImageAssets(
    job: GenerationJob,
    result: unknown,
    outputFormat: string,
  ) {
    ...
    const items = Array.isArray(record.data)
      ...
      : Array.isArray(record.images)
        ...
        : [];
    const urls = items
      .map((item) => String(asRecord(item).url || "").trim())
  ...
  private async persistGeneratedImages(
    job: GenerationJob,
    result: unknown,
    assets: Array<{ id: string; publicId: string; ordinal: number }>,
  ) {
    ...
    for (let offset = 0; offset < assets.length; offset += concurrency) {
      ...
      const outcomes = await Promise.allSettled(
        batch.map(async (asset) => {
          ...
          const stored = await this.canvasMedia.saveBufferForWorkspace(
            job.userId,
            job.workspaceId,
            storageKey,
            media.contentType,
      ...
      outcomes.forEach((outcome, index) => {
        if (outcome.status === "rejected") {
          ...
          this.logger.warn(
            `图片资产 ${asset?.publicId || "unknown"} 持久化失败：${outcome.reason instanceof Error ? outcome.reason.message : "未知错误"}`,
          );
        }
      });
    }
  ...
  private async registerVideoAsset(job: GenerationJob, sourceUrl: string) {
    if (!/^https?:\/\//i.test(sourceUrl))
      throw new BadRequestException("视频结果地址无效");
    const existing = await this.prisma.asset.findFirst({
      where: { generationJobId: job.id, kind: AssetKind.VIDEO, ordinal: 0 },
      select: { data: true },
    });
    ...
    if (
      existingStorageKey &&
      String(existingData.url || "").startsWith("/platform-api/canvas-media/")
    )
      return String(existingData.url);
    const media = await this.upstream.videoMedia(sourceUrl);
  ...
  private async registerAssets(
    ...
    try {
      return await Promise.all(
        outputs.map((output, ordinal) =>
          this.prisma.asset.upsert({
            where: {
              generationJobId_ordinal: { generationJobId: job.id, ordinal },
            },
            create: {
...
export function rewriteGeneratedImageUrls(
  result: unknown,
  assets: Array<{ publicId: string; ordinal: number }>,
) {
  const record = asRecord(result);
  ...
  const rewrite = (items: unknown[]) =>
    items.map((item, index) => {
      const proxyUrl = proxyByOrdinal.get(index);
      const value = asRecord(item);
      return proxyUrl && typeof value.url === "string"
        ? { ...value, url: proxyUrl }
        : item;
    });
  return {
...
function validateIdempotencyKey(value: string) {
  if (!/^[A-Za-z0-9._:-]{8,160}$/.test(value))
    throw new BadRequestException("Idempotency-Key 缺失或格式无效");
}
...
function summarizeVideo(input: VideoRequest): Prisma.InputJsonValue {
  return {
    ...
    imageReferences: input.images.length,
    videoReferences: input.videos.length,
    audioReferences: input.audios.length,
  };
}

function summarizeSpeech(input: SpeechRequest): Prisma.InputJsonValue {
  return {
    textHash: hashText(input.input),
    characterCount: input.input.length,
    responseFormat: input.response_format,
    voiceConfigured: Boolean(input.voice),
    speed: input.speed,
  };
}
...
function publicVideoTaskResult(
  ...
  return {
    ...publicResult,
    id: publicId,
    ...(resultUrl ? { result_url: resultUrl, is_final: true } : {}),
  };
}
export function videoResultUrl(result: Record<string, unknown>) {
  const nestedData = Array.isArray(result.data)
    ? result.data.map(asRecord)
...
function videoFailed(result: Record<string, unknown>, state: string) {
  return (
    ["failed", "failure", "cancelled", "canceled", "expired"].includes(state) ||
    (Boolean(result.is_final) && !videoResultUrl(result))
  );
}
function readResultError(result: Record<string, unknown>) {
...
function imageMimeType(format: string) {
  return format === "jpg" ? "image/jpeg" : `image/${format || "png"}`;
}
...
function storedVideoFile(
  ...
  return {
    url: canvasMediaUrl(storageKey, checksum),
    storageKey,
    bytes,
...
export function publicVideoGenerationError(
  error: unknown,
  input: Pick<VideoRequest, "model" | "videos" | "audios">,
) {
  ...
  return new ServiceUnavailableException(
    `MetaJing 当前没有为「${selected?.label || input.model}」开放生成通道。本次未扣费，预授权已自动退回；请切换到列表中的另一个 Seedance 通道后重试。`,
  );
}
