...
test("账户资产列表始终附带当前 userId 条件", async () => {
    ...
    await service.list("user-a", { page: 1, limit: 100 });
    assert.deepEqual(capturedWhere, { userId: "user-a" });
});

test("不同账户不能覆盖同一资产编号", async () => {
    const prisma = { asset: { findUnique: async () => ({ id: "asset-db-id", userId: "user-a" }) } } as unknown as PrismaService;
    const service = new AssetService(prisma);
    await assert.rejects(() => service.upsert("user-b", "asset_public_01", validInput), ConflictException);
});

test("资产 API 拒绝把大媒体内容塞进 JSON 元数据", async () => {
    const prisma = { asset: { findUnique: async () => null } } as unknown as PrismaService;
    const service = new AssetService(prisma);
    await assert.rejects(() => service.upsert("user-a", "asset_public_02", { ...validInput, data: { content: "x".repeat(101 * 1024) } }), BadRequestException);
});
