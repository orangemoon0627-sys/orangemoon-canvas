...
+ [调整] 复核 MetaJing 官方价格后，视频目录严格收敛为 Seedance 2.0 Fast 和 Seedance 2.0 两个独家产品；用户界面不再显示供应商内部前缀，431 与其他旧型号不再展示、报价、预授权或调用上游，旧 431 浏览器配置单向迁移到清衍标准版。
...
+ [修复] MetaJing Image 2 图生图统一改用 `/v1/images/generations` 的 `image` 字段，不再请求不兼容的图片编辑端点。
