...
- 外部服务请求统一放在 `web/src/services/api/`。橙月官方渠道必须经过服务端供应商网关，不能把供应商密钥下发浏览器；用户自定义渠道继续由浏览器直连。
...
- 画布项目、节点/连线、助手上下文和“我的素材”元数据按账户同步到 PostgreSQL；媒体文件同步到服务器持久化卷，浏览器 `localforage` 仅作本地缓存。文档中要区分当前单机持久化卷与尚未接入的 S3/OSS 对象存储。
- 橙月官方渠道的供应商密钥只允许保存在 Keychain、环境变量或服务器密钥管理服务；自定义渠道的 API Key 仍存在浏览器本地并由前端直连，涉及安全说明时要区分两者。
- Docker 静态资源与同源平台 API 已完成构建验证；共享低内存主机部署必须保留请求体、生成并发和容器资源限制。
