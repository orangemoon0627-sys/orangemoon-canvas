...
export function ConnectionCreateMenu({
    ...
    return (
        <div
            className="absolute z-[120] w-[236px] rounded-lg border p-1.5 shadow-2xl backdrop-blur"
        ...
        >
            <div className="mb-1 flex h-8 items-center justify-between px-2">
                ...
                <button type="button" className="grid size-7 place-items-center rounded-md text-base opacity-55 transition hover:bg-white/10 hover:opacity-100" onClick={onClose} aria-label="关闭">
                    ×
                </button>
            </div>
...
export function ConnectionCreateOption({ theme, icon, title, description, onClick }: { theme: (typeof canvasThemes)[keyof typeof canvasThemes]; icon: React.ReactNode; title: string; description?: string; onClick?: () => void }) {
    return (
        <button
            type="button"
            className="flex min-h-10 w-full cursor-pointer items-center gap-2 rounded-md px-2 py-1.5 text-left transition"
            style={{ color: theme.node.text }}
            onClick={onClick}
            onMouseEnter={(event) => (event.currentTarget.style.background = theme.node.fill)}
            onMouseLeave={(event) => (event.currentTarget.style.background = "transparent")}
        >
            <span className="grid size-8 shrink-0 place-items-center rounded-md [&>svg]:size-4" style={{ background: theme.node.fill, color: theme.node.muted }}>
                {icon}
            </span>
            <span className="min-w-0 flex-1">
                <span className="flex items-center gap-2 text-xs font-medium leading-4">{title}</span>
                {description ? (
                    <span className="mt-0.5 block truncate text-[10px] leading-3" style={{ color: theme.node.muted }}>
                        {description}
                    </span>
                ) : null}
            </span>
...
export function NodeCreateMenu({ position, onCreate, onClose }: { position: Position; onCreate: (type: string) => void; onClose: () => void }) {
    ...
    return (
        <div
            ref={menuRef}
            className="absolute z-[120] max-h-[70vh] w-[236px] overflow-y-auto rounded-lg border p-1.5 shadow-2xl backdrop-blur thin-scrollbar"
            data-canvas-no-zoom
