...
const QY_25_REFERENCES = { images: 30, videos: 10, audios: 10, imageMaxBytes: 30_000_000, videoMaxBytes: 200_000_000, audioMaxBytes: 15_000_000, audioMaxItemSeconds: 15, audioMaxTotalSeconds: 150, videoMinShortEdge: 720, videoMaxShortEdge: 2160, videoMaxLongEdge: 2160 };
...
export const PROVIDER_MODELS: ProviderModel[] = [
    { id: "gpt-image-2", label: "Image 2", provider: "metajing", capability: "image", visibility: "public", billing: { unit: "image", usd: 0.06 }, description: "文生图与最多 4 张参考图融合，支持最长边 3840px，一次最多生成 4 张" },
    videoModel("431-Seedream-2.0-fast", "Seedance 2.0 Fast（431）", { "480p": 0.136, "720p": 0.189 }, "720p", REF_431, { tier: "fast", maxPromptChars: 10_000, minDuration: 5, maxDuration: 14, allowedDurations: [5, 10, 14], recommendedDurations: [5, 10, 14], aspectRatios: RATIOS_431, supportsFrames: true, supportsEndFrame: true, description: "431 Fast 通道，支持 480P/720P、原生声音及最多 4 图/3 视频/1 音频参考" }),
    videoModel("431-Seedream-2.0", "Seedance 2.0（431）", { "480p": 0.148, "720p": 0.215 }, "720p", REF_431, { tier: "standard", maxPromptChars: 10_000, minDuration: 5, maxDuration: 14, allowedDurations: [5, 10, 14], recommendedDurations: [5, 10, 14], aspectRatios: RATIOS_431, supportsFrames: true, supportsEndFrame: true, description: "431 标准通道，支持 480P/720P、原生声音及最多 4 图/3 视频/1 音频参考" }),
    videoModel("qy-seedance-2.5", "Seedance 2.5", { "480p": 0.19, "720p": 0.24 }, "720p", QY_25_REFERENCES, { tier: "standard", maxPromptChars: 10_000, minDuration: 4, maxDuration: 29, recommendedDurations: [5, 10, 15, 29], aspectRatios: RATIOS_431, videoReferenceMultiplier: 1.6, supportsFrames: true, supportsEndFrame: true, framesConflictWithImages: true, description: "独家 2.5 通道，支持 4-29 秒任意整数、默认声音及最多 30 图/10 视频/10 音频参考；带参考视频整单加价 60%" }),
    videoModel("qy-seedance-2.0-fast", "Seedance 2.0 Fast", { "480p": 1 / 6, "720p": 0.2 }, "720p", QY_REFERENCES, { tier: "fast", maxPromptChars: 2_500, minDuration: 5, maxDuration: 15, allowedDurations: [5, 10, 15], recommendedDurations: [5, 10, 15], aspectRatios: QY_RATIOS, description: "Fast 通道，支持 480P/720P、原生声音及图片、视频、音频参考" }),
    videoModel("qy-seedance-2.0", "Seedance 2.0", { "480p": 0.2, "720p": 4 / 15, "1080p": 0.6 }, "720p", QY_REFERENCES, { tier: "standard", maxPromptChars: 2_500, minDuration: 5, maxDuration: 15, allowedDurations: [5, 10, 15], recommendedDurations: [5, 10, 15], aspectRatios: QY_RATIOS, description: "标准通道，支持 480P/720P/1080P、原生声音及图片、视频、音频参考" }),
...
export function estimateProviderCost(model: ProviderModel, quantity = 1, resolution?: string, options: { hasVideoReferences?: boolean } = {}) {
    ...
    const baseUpstreamUsd = billing.unit === "million_characters" ? (billing.usd * normalizedQuantity) / 1_000_000 : billing.usd * normalizedQuantity;
