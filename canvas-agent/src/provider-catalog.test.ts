...
test("provider catalog exposes only the four exclusive video APIs", () => {
    ...
    assert.equal(PUBLIC_PROVIDER_MODELS.length, 7);
...
test("provider cost conversion keeps MetaJing quota and MiniMax USD rates separate", () => {
    ...
    const video = findProviderModel("Seedance 2.0-fast-720p");
    ...
    assert.deepEqual(estimateProviderCost(video, 15, "720p"), { upstreamUsd: 1.5, upstreamCny: 1.5, suggestedCredits: 2.475, resolution: "720p" });
...
test("Seedance reference limits remain model-specific", () => {
    const exclusive = findProviderModel("Seedance 2.0-fast-720p");
