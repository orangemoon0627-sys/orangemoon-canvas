...
当前画布项目采用本地缓存与账户云端主副本并行保存：

- 本地缓存：`localForage`，数据库名 `infinite-canvas`，storeName `app_state`，key 为 `infinite-canvas:canvas_store`。
- 云端项目：PostgreSQL `CanvasProject`，按 `userId + publicId` 唯一并使用客户端更新时间解决整项目快照冲突；删除保留墓碑。
- 云端媒体：PostgreSQL `CanvasMedia` 保存索引，原始图片/视频/音频写入服务器持久化卷；新设备按 `storageKey` 懒加载回本地缓存。
- Agent 对话：PostgreSQL `AgentThread`，按 `userId + projectId` 隔离，画布项目 ID 在同步时保持不变。
