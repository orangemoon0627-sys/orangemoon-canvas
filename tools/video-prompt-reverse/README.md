...
默认使用 `standard` 档覆盖完整时间线，但视觉分析最多发送其中 8 帧；一分钟左右的多场景短片可以兼顾覆盖率和反馈速度。只需要粗略概要时可显式使用 `--profile fast`，关键动作仍不清楚时再使用 `--profile detailed`，不需要无条件追加逐秒帧和切点帧。

抽帧档位：

| 档位 | 帧数 |
| --- | --- |
| `fast` | 固定 5 帧，适合主动选择的极速预览 |
| `standard` | `<=10s` 6 帧，`<=30s` 10 帧，`<=60s` 14 帧，更长 16 帧 |
| `detailed` | 对应 10 / 16 / 24 / 32 帧 |

最后一帧取视频约 95% 的时刻，减少片尾黑帧。默认最长边压到 960px，视觉分析最多发送 8 帧，以控制反馈时间、请求体和成本。
...
`seedance-prompt.md` 中的还原提示词只用于理解参考片结构，不建议直接发布。原创改写应复用已有反推结果，只保留构图、节奏、主运镜、转场和材质逻辑，至少替换五类核心创意元素；不需要重新下载视频或重新抽帧。
